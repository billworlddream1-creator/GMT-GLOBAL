...
const NewsPanel: React.FC<NewsPanelProps> = ({ item, position, onSelect }) => {
  ...
  return (
    <Float speed={1.5} rotationIntensity={0.2} floatIntensity={0.5} position={position}>
      <group 
        onClick={(e) => { e.stopPropagation(); playUISound('success'); onSelect(item.image || '', item.title); }}
        onPointerOver={() => (document.body.style.cursor = 'pointer')}
        onPointerOut={() => (document.body.style.cursor = 'auto')}
      >
        <RoundedBox args={[4, 2.5, 0.1]} radius={0.1} smoothness={4}><meshBasicMaterial map={texture} /></RoundedBox>
        <RoundedBox args={[4.2, 2.7, 0.05]} radius={0.1} smoothness={4} position={[0, 0, -0.06]}><meshBasicMaterial color="#3b82f6" transparent opacity={0.2} /></RoundedBox>
        <Text position={[0, -1.8, 0]} color="white" fontSize={0.15} maxWidth={3.5} textAlign="center" font="https://fonts.gstatic.com/s/orbitron/v25/yMJ4DifL97qlqHuxv4p5pXP68UFID_jI.woff">{item.title.toUpperCase()}</Text>
      </group>
    </Float>
  );
};
...
const SpatialLab: React.FC<SpatialLabProps> = ({ news, onSelect }) => {
  ...
  useEffect(() => {
    // Perform check immediately but after initial mount
  ...
  if (hasWebGL === null) {
    return (
      <div className="w-full h-full flex items-center justify-center glass rounded-[3rem] bg-black/20">
      ...
      <div className="w-full h-full glass rounded-[3rem] overflow-hidden flex flex-col p-10 border border-white/10 bg-black/40 animate-in fade-in">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayNews.map((item) => (
            <div key={item.id} onClick={() => onSelect(item.image || '', item.title)} className="glass rounded-3xl overflow-hidden border border-white/5 hover:border-accent transition-all cursor-pointer group">
              ...
              <div className="p-4 bg-slate-900/60"><h4 className="text-[9px] font-heading font-black text-white uppercase leading-tight tracking-widest">{item.title}</h4></div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="w-full h-full glass rounded-[3rem] overflow-hidden relative border border-white/10 bg-black/40">
      ...
      <div className="absolute bottom-10 right-10 z-10 glass px-6 py-3 rounded-2xl border border-white/10 text-[9px] font-mono text-slate-500 uppercase tracking-widest pointer-events-none">Scroll to Zoom • Drag to Rotate • Click Image for VR View</div>
        ...
        <Suspense fallback={null}>
          ...
          <group>
            {displayNews.map((item, i) => {
              ...
              return <NewsPanel key={item.id} item={item} position={[x, y, z]} onSelect={onSelect} />;
            })}
          </group>
          <OrbitControls enablePan={false} minDistance={5} maxDistance={25} autoRotate autoRotateSpeed={0.2} />
        </Suspense>
      </Canvas>
    </div>
  );
